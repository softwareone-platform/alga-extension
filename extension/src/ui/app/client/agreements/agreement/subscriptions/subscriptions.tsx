...
const columns: ColumnDef<SubscriptionRow>[] = [
  {
    ...
    cell: ({ row: { original } }) => (
      <div className="flex flex-col gap-0.5 items-start">
        <span className="block truncate text-blue-500 hover:text-blue-600 w-full">
          {original.name}
        </span>
        <span className="block truncate text-xs text-text-500 w-full">
          {original.id}
        </span>
      </div>
    )
    ...
    <Card>
      <TableContainer>
        <div className="w-full overflow-x-scroll relative">
          <table style={{ width: '100%', minWidth: table.getTotalSize() }} className="relative table-fixed">
            <thead>
