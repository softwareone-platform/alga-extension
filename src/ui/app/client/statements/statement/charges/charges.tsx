...
const createColumns = (currency?: string): ColumnDef<ChargeRow>[] => [
  {
    accessorKey: 'id',
    header: 'Name',
    minSize: 160,
  ...
  },
  {
    ...
    size: 150,
    cell: ({ row: { original } }) => (
      <span className="truncate block">{original.period?.start ? formatDateTime(original.period.start) : "—"}</span>
    )
  },
  {
    accessorKey: 'endDate',
    ...
    size: 150,
    cell: ({ row: { original } }) => (
      <span className="truncate block">{original.period?.end ? formatDateTime(original.period.end) : "—"}</span>
    )
  },
  {
  ...
  },
  {
    ...
    size: 128,
    cell: ({ row: { original } }) => (
      <PriceWithMarkup currency={currency} value={original.price?.SPx1} markup={original.billingConfig?.markup} />
    )
  },
];
...
export function Charges() {
  ...
  const columns = createColumns(statement?.price?.currency?.sale);
