...
export const Component = forwardRef<HTMLElementType, ComponentProps>(
  ({ className, ...props }, ref) => {
    return <Element ref={ref} className={clsx(baseStyles, className)} {...props} />;
  }
);
...
- **CSS Custom Properties**: Theme variables (e.g., `rgb(var(--color-primary-600))`)
...
Theme variables are accessed through CSS custom properties:
- `--color-primary-*` - Primary color scale
- `--color-text-*` - Text color variations  
- `--color-border-*` - Border color variations

Usage: `rgb(var(--color-primary-600))` for proper opacity support
